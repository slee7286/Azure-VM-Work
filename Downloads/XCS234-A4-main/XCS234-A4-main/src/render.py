    ...
    if args.checkpoint:
        if args.dpo:
            ...
            with open(args.checkpoint, "rb") as f:
                agent: Union[DPO, SFT] = torch.load(f, weights_only=False)
        ...
        for _ in range(T):
            ...
            if done:
                break
        clip = ImageSequenceClip(images, fps=int(1 / env.unwrapped.dt))
        ...
        if data["label"][idx].item() == 0.5:
            name1 = f"{int(data["label"][idx].item())}_{idx}_equally_preferred1.mp4"
            name2 = f"{int(data["label"][idx].item())}_{idx}_equally_preferred2.mp4"
        else:
            name1 = [
                f"{int(data["label"][idx].item())}_{idx}_preferred.mp4",
                f"{int(data["label"][idx].item())}_{idx}_not_preferred.mp4",
            ][int(data["label"][idx].item())]
            name2 = [
                f"{1 - int(data["label"][idx].item())}_{idx}_preferred.mp4",
                f"{1 - int(data["label"][idx].item())}_{idx}_not_preferred.mp4",
